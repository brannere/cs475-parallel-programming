...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
            ...
            // send information from one node to another
            // let the receiver know what the incoming message is for
            //                              (elements)       (cpu rank)
            //      arr,                    numToSend, type, dst, tag, MPI_COMM_WORLD
			MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXSHIFTS, MPI_FLOAT, dst, 'a', MPI_COMM_WORLD );
		}
	}
	else
	{
		MPI_Recv( PPSignal, PPSize+MAXSHIFTS, MPI_FLOAT, BOSS, 'a', MPI_COMM_WORLD, &status );
	}
	...
	{
		MPI_Send( PPSums, MAXSHIFTS, MPI_FLOAT, BOSS, 'b', MPI_COMM_WORLD );
	}

    if(DEBUG) fprintf(stderr, "Node %3d done each processor sends its usms back to the BOSS\n", me);
	// receive the sums and add them into the overall sums:

	if( me == BOSS )
	{
        if(DEBUG) fprintf(stderr, "I'M THE BOSS with %d cpus\n", NumCpus);
		float tmpSums[MAXSHIFTS];
		for( int src = 0; src < NumCpus; src++ )
		{
            if(DEBUG) fprintf(stderr, "src: %d\n", src);
			if( src == BOSS )
				continue;

            if(DEBUG) fprintf(stderr, "before recv\n");
			MPI_Recv( tmpSums, MAXSHIFTS, MPI_FLOAT, src, 'b', MPI_COMM_WORLD, &status );
            if(DEBUG) fprintf(stderr, "before recv\n");
            for( int s = 0; s < MAXSHIFTS; s++ ){
    ...
    if(DEBUG) fprintf(stderr, "Node %3d Done recieve the sums and add them into overall sums\n", me);
	// stop the timer:
...
DoOneLocalAutocorrelation( int me )
{
    ...
    if(DEBUG) fprintf(stderr,"Node %3d returning from DoOneLocalAutocorrelation()\n",me);
}
